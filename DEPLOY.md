...
- **Manage a node's users and protocols from the main panel.** On the **Nodes** page, click **Manage** on a reachable node to create/edit/enable/disable its users and toggle, add or remove its protocols (inbounds), without signing in to the node. The panel drives it over the node's REST API with the token minted at enrollment.
